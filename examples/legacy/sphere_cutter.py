    ...
    def cut(self, rad, sphereCenter=None):
        ...
        if not isinstance(rad, float):
            try:
                ...
                raise TypeError("the radius of a sphere must be expressed as a number") from None
        if not sphereCenter:
            sphereCenter = self.findCenter(vectors)
        else:
            if len(sphereCenter) != 3 or not isinstance(sphereCenter, tuple):
                raise TypeError(
