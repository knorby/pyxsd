...
crash. A former crash must become either a structured validation report
...
def _parse(tmp_path, schema_text, instance_text, mode=ParseModes.STRICT):
    ...
    return Schema.compile(str(schema), mode=mode).parse(str(instance))


def _parse_schema(tmp_path, schema_text):
    """Compiles an inline schema without an instance document."""
    schema = tmp_path / "schema.xsd"
    schema.write_text(schema_text)
    return Schema.compile(str(schema))
...
class TestUntypedAttributeDefaultsToAnySimpleType:
    ...
    def test_getType_is_anySimpleType(self, tmp_path):
        ...
        assert doc.schema.classes["t"].note.getType() is AnySimpleType
...
class TestListTypedAttributeBinding:
    ...
    def test_list_attribute_binds_items(self, tmp_path):
        doc = _parse(tmp_path, LIST_SCHEMA, '<t version="1.0 1.1"/>')
        assert doc.report.has_errors is False
        root = doc.root
        assert [str(item) for item in root.version] == ["1.0", "1.1"]

    def test_list_attribute_single_token(self, tmp_path):
        doc = _parse(tmp_path, LIST_SCHEMA, '<t version="1.1"/>')
        assert doc.report.has_errors is False

    def test_list_item_type_validates(self, tmp_path):
        doc = _parse(tmp_path, LIST_SCHEMA, '<t version="9.9"/>')
        assert doc.report.has_errors is True
    ...
    def test_list_length_facet_counts_items(self, tmp_path):
        doc = _parse(tmp_path, LIST_SCHEMA, '<t pair="1.0 1.1 1.0"/>')
        assert doc.report.has_errors is True
        codes = [issue.code for issue in doc.report.issues]
        assert "invalid-attribute" in codes
    ...
    def test_xsts_metaschema_version_list_does_not_crash(self):
        doc = Schema.compile(
            str(XSTS_CORPUS / "common" / "xsts.xsd"), mode=ParseModes.NAMESPACED
        ).parse(str(XSTS_CORPUS / "saxonMeta" / "All.testSet"))
        root = doc.root
...
class TestAttributeRefResolution:
    ...
    def test_ref_to_untyped_global_attribute(self, tmp_path):
        ...
        assert doc.report.has_errors is False
        assert str(doc.root.att2) == "anything"
...
class TestAnnotationContent:
    ...
    def test_documentation_text_still_sets_doc(self, tmp_path):
        ...
        assert doc.report.has_errors is False
        assert doc.schema.classes["t"].__doc__ == "hello docs"
...
class TestUnnamedDeclarations:
    ...
    def test_global_attribute_without_name(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"><xs:attribute/></xs:schema>',
        )
        codes = [issue.code for issue in schema.report.issues]
        assert "declaration-name" in codes

    def test_local_attribute_with_empty_name(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
    ...
    def test_complex_type_with_empty_name(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
...
class TestCircularDerivation:
    ...
    def test_self_restriction_reports(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
    ...
    def test_self_extension_reports(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
    ...
    def test_indirect_cycle_reports(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
...
class TestMisplacedDeclarations:
    ...
    def test_top_level_group_ref(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
        ...
        assert "misplaced-declaration" in _codes(schema.report)

    def test_compositor_in_simple_type(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
        ...
        assert "declaration-child" in _codes(schema.report)

    def test_attribute_in_group(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
    ...
    def test_group_ref_in_simple_type(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
    ...
    def test_group_ref_in_attribute_group(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
    ...
    def test_nested_attribute_group_definition(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
    ...
    def test_attribute_group_definition_in_extension(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
...
class TestExtensionWithoutBase:
    ...
    def test_missing_base_reports(self, tmp_path):
        ...
        assert "extension-base" in _codes(schema.report)
...
class TestConflictingListRestriction:
    ...
    def test_list_with_restriction_reports(self, tmp_path, monkeypatch, children):
        ...
        assert "conflicting-derivation" in _codes(schema.report)
...
class TestNotationSupport:
    ...
    def test_notation_declaration(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
        ...
        assert schema.report.has_errors is False

    def test_bare_notation_attribute_reports_not_crashes(self, tmp_path):
        ...
        assert "notation-enumeration-required" in _codes(schema.report)

    def test_nested_notation_reports(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
            '<xs:complexType name="c"><xs:attribute name="foo">'
    ...
    def test_notation_restriction_needs_enumeration(self, tmp_path):
    ...
    def test_derived_restriction_may_add_facets(self, tmp_path):
    ...
    def test_notation_enumeration_must_be_declared(self, tmp_path):
        ...
        assert "unknown-notation" in _codes(schema.report)

    def test_imported_notation_resolves(self, tmp_path):
    ...
    def test_declared_notation_enumeration_ok(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
            '<xs:notation name="jpeg" public="image/jpeg" system="viewer.exe"/>'
...
class TestUnreadableInputs:
    ...
    def test_missing_instance_file(self, tmp_path):
        ...
        schema_obj = Schema.compile(str(schema))
        with pytest.raises(PyXSDError):
            schema_obj.parse(str(tmp_path / "missing.xml"))
...
class TestOversizedPatternRepetition:
    ...
    def test_reports_facet_error(self, tmp_path):
        schema = _parse_schema(
            tmp_path,
            '<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">'
