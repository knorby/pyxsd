...
write a standard xml tree given a standard set of
variables. XmlTreeWriter must be passed a root element, which is the
highest level element in an xml tree.  This element must contain in
its dictionary the following variables:
    
*_name_* : String

    A string that is the name of the element. The name will appear in
    as the first word after the '<' symbol. If the name is set to
...
*_attribs_* : Dictionary
    ...
    the attributes values. The values will have the str() function
...
*_value_* : List or NoneType
    ...
    either a list or None. The values should be in a from that will
    easily be converted to a string. The value for a given element
    should be its non-element child. Each value will be an entry in
    the list. These values are commonly numbers.
...
class XmlTreeWriter(object):
    ...
    def passTagToTagWriter(element, tabs, output):
        """Extracts element variables and initializes the tag writer for the
        element.  Recursively calls itself on its element
        children. Writes the ending tag if it has any values or any
        children.
        ...
        - `tabs`: An integer that specifies how many tabs preceed an
    ...
    def writeHeaderInfo(self):
        ...
        information. Includes data and time information.
