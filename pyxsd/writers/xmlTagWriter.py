class XmlTagWriter(object):
    """This class writes one element. Each tag has its own instance of
    this class. it contains a function to write the end tag if the
    element has children, but this function is called from the tree
    writer. See XmlTreeWriter for the API and other information. This
    class should only be initialized by XmlTreeWriter.
    ...
    def __init__(self, name, attribs, value, hasChildren, hasValue, tabs, output):
        ...
        - `value`: A list of the element values or None if there are
          no values
        ...
        self.sortedKeyList = sorted(self.attribs.viewkeys())
        self.value = value
    ...
    def writeTag(self):
        ...
        non-necessary formatting is standard and is not dependant apon
    ...
    def writeComment(self):
        """If `name` is set to '_comment_' this function is called. A comment
        can be in the tree only if it is included in a transform or
        the writer is used by another program.
    ...
    def writeEndTag(self):
        """Writes the ending tag for an element. Called by the tree writer if
        the element has children.  It is called only after the other
    ...
    def writeTabs(self, tabSpec=None, tabs=None):
        """Writes out the tabs before an element. Can also write a certain
        number of spaces after the tabs have been written, if
        `tabSpec` is specified.

        parameters:

        - `tabSpec`: An integer. By default, it is a NoneType. If
          specified, the program will make the specified number of
          spaces after the tab it writes.

        - `tabs`: n integer that indicates the number of tabs over the
          element is in the document. By default, it is set to
