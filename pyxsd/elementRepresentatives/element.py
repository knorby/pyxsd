...
class Element(ElementRepresentative):
    """The class for the element tag. Subclass of
    *ElementRepresentative*.  The element tag and the attribute tag
    are the most important in the xml and in the program, so this
    class contains some machinery that many of the other classes do
    not have. The element and attribute classes contain descriptor
    methods. By specifing the `__get__`, `__set__`, and `__delete__`
    (with `__get__` and `__set__` being the most important). These
    methods specify how a variable is set and how it retrieved. Any
    modification of these methods should be made under extreme
    caution! Both Element and Attribute, primarily Attribute, use
    these descriptors to add a level oof checking to the program. If
    some variable is set to some value that does no match the
    specifications in the schema, an error will be raised. These
    ...
    def __init__(self, xsdElement, parent):
        """Adds itself to the element list in its parent.  See
        *ElementRepresentative* for documentation.
    ...
    def getType(self):
        ...
        while the classes for the schema types are being
        built. Clearly, this function is used after the main ER run.
    ...
    def __str__(self):
        """Prints its name in a form that allows for quick identification of
        an element, without needing a bulky name that does not match
        the name used.
    ...
    def __get__(self, obj, mystery=None):
        ...
        anything, so it should not really be thought of as a parameter
        to this method. This variable is needed because of, you
        guessed it, a mystery. Sometimes, but not every time, the obj
        is passed in as an instance and it corresponding
        class. Perhaps it is slightly different, but it does this very
        odd thing with only some of the attributes/elements.  As far
        as I can tell, the 'mystery' does not cause any problems, but
        it is a bug, and should be fixed if possible.

        See python documentation for full documentation on
        descriptors. Some links should be included on the `pyXSD
        website <http://pyxsd.org>`_.
    ...
    def __set__(self, obj, value):
        """Sets an element's name to the element in the obj's dictionary.  If
        multiple elements exist, sets it to a list. If it is not an
        element, raises an error. Has code for case when it is
        dictionary, but no case in which a dictionary would be used.
    ...
    def isList(self):
        """Returns true if maxOccurs is greater than one. If it is true,
        treats all of the elements that are from the schema definition
        as a list. Otherwise returns false.
