...
def compile_content_model(type_er: Any, py_xsd: Any = None) -> Particle | None:
    ...
    own = _group_particles(_compile_items(content, type_er, frozenset()))
...
def _compile_items(items: list[Any], owner: Any, visited: frozenset[str]) -> list[Particle]:
    particles: list[Particle] = []
    for item in items:
        particle = _compile_item(item, owner, visited)
...
def _compile_item(item: Any, owner: Any, visited: frozenset[str]) -> Particle | None:
    className = type(item).__name__
    if className == "Group":
        if getattr(item, "isRefSite", False):
            return _compile_group_ref(item, owner, visited)
        return None
    if className == "Element":
        return _compile_element(item)
    if className in ("Sequence", "Choice", "All"):
        minimum, maximum = _occurrence(getattr(item, "tagAttributes", {}) or {})
        children = _compile_items(_content_children(item), owner, visited)
        return Particle(className.lower(), minimum, maximum, children)
    return None


def _compile_element(item: Any) -> Particle | None:
    name: str | None
    if getattr(item, "isElementRef", False):
...
def _compile_group_ref(ref_site: Any, owner: Any, visited: frozenset[str]) -> Particle | None:
    ref = str(getattr(ref_site, "ref", ""))
    if not ref or ref in visited:
        return None
    schema = owner.getSchema()
    group = schema.groups.get(ref) if schema is not None else None
    if group is None:
        return None
    compositor = group.getCompositor()
    if compositor is None:
        return None
    inner = _compile_item(compositor, owner, visited | {ref})
    if inner is None:
        return None
...
def _match_one(
    ...
    if particle.is_element():
        if position < len(nodes) and _accepts(_name_of(nodes[position]), particle, member_head_map):
    ...
    if particle.kind == "sequence":
        ...
        for child in particle.children:
            ...
            for start, matched in states:
                for end, more in _match_repeated(child, nodes, start, member_head_map, depth + 1):
    ...
    if particle.kind == "choice":
        ...
        for branch in particle.children:
            results.extend(_match_repeated(branch, nodes, position, member_head_map, depth + 1))
        return _dedup(results)
    if particle.kind == "all":
        return _match_all(particle, nodes, position, member_head_map)
    return []
...
def _match_all(
    ...
    while index < len(nodes):
        node_name = _name_of(nodes[index])
        chosen = None
...
def _match_repeated(
    ...
    while frontier and count < maximum:
        ...
        for start, matched in frontier:
            for end, more in _match_one(particle, nodes, start, member_head_map, depth):
                if end == start:
                    continue
                advanced.append((end, matched + more))
...
def match_content(
    ...
    states = _match_repeated(model, nodes, 0, member_head_map, 0)
    for end, _ in states:
        if end == len(nodes):
            return True, []
