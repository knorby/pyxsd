...
| `unexpected-character` | ERROR | A complex-typed element whose content model is element-only has character content other than whitespace (XSD 1.1 §3.4.3.2, Element Locally Valid (Complex Type)). Mixed content types and simple content are not checked (their text is legal or is the value), and neither are elements whose content model could not be compiled (the legacy fallback keeps its historical tolerance). |
