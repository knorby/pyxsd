...
| `nil` | ERROR | `xsi:nil="true"` on a non-nillable element, or an `xsi:nil` value outside the boolean lexical space (`true`/`false`/`1`/`0`, case-insensitively; surrounding whitespace is tolerated). The value is checked even when an attribute wildcard admits the xsi namespace, because the built-in declaration is typed `xs:boolean` (XSD 1.1 §3.2.7.2). |
