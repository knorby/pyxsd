...
| `open-content-invalid` | ERROR | An `xs:openContent` declaration violates its XML representation (XSD 1.1 §3.4.2.2/§3.4.3): the `mode` is not `none`/`interleave`/`suffix`; `mode="none"` carries an `xs:any` child, or a non-`none` mode has other than exactly one `xs:any` child; or one complex type carries more than one `xs:openContent`. An illegal child wildcard is reported through the shared wildcard code (`wildcard-invalid`/`invalid-attribute`), not here. |
