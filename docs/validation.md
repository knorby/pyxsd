...
| `substitution-type` | ERROR | A substitution-group member's declared type is not validly derived from its head's type (XSD 1.1 §3.3.5.2). Reported only for a member with more than one head: single-head mismatches stay permitted so the shipped `tests/fixtures/substitution` fixture (an `xs:integer` member under an `xs:string` head) keeps its historical clean report. |
