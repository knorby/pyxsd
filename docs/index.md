# pyxsd documentation

Documentation for **pyxsd** 1.0.0 — a Python library and CLI with one small,
pure-Python dependency (`elementpath`, for XSD regular expressions and
XPath) that maps XML documents into Python object trees according to an XML
Schema (XSD), reports non-fatal validation issues, queries and exports the
bound tree (`xpath` / `find` / `to_dict` / `to_json`), runs user-defined
*transforms*, and writes the tree back out as XML. It is an XSD 1.1
processor with an optional XSD 1.0 mode.

```{toctree}
:maxdepth: 2
:caption: Contents

quickstart
cli
demo
architecture
data-model
validation
binding
transforms/index
supported
comparison-xmlschema
api
migration-1.0
history/index
contributing
```

```{toctree}
:maxdepth: 1
:caption: Project

License <https://github.com/knorby/pyxsd/blob/develop/LICENSE>
Source <https://github.com/knorby/pyxsd>
```

## Indices and tables

- {ref}`genindex`
- {ref}`modindex`
- {ref}`search`
